...
function authResult(u: MockUser): Record<string, unknown> {
  return {
...
function publicCommunityUser(u: MockUser): Record<string, unknown> {
  return {
    id: u.id,
    nickname: u.nickname,
    avatarUrl: u.avatarUrl,
...
function isPubliclyVisible(post: MockPost): boolean {
  const author = users.find((u) => u.id === post.userId)
  return post.visibility === 'PUBLIC' && post.status === 'VISIBLE' && author?.status === 'NORMAL'
}

function publicPost(post: MockPost, currentUserId: number): Record<string, unknown> {
  const author = users.find((u) => u.id === post.userId)
  return {
    id: post.id,
    author: author ? publicCommunityUser(author) : null,
    checkInId: post.checkInId,
    content: post.content,
    visibility: post.visibility,
    status: post.status,
    likeCount: postLikes.filter((item) => item.postId === post.id).length,
    commentCount: comments.filter((item) => item.postId === post.id && item.status === 'VISIBLE').length,
    likedByCurrentUser: postLikes.some((item) => item.postId === post.id && item.userId === currentUserId),
    createdAt: post.createdAt,
    updatedAt: post.updatedAt,
  }
}

function publicComment(comment: MockComment): Record<string, unknown> {
...
function requireCommunityWriter(res: ServerResponse, u: MockUser | null): MockUser | null {
  if (!u) {
    fail(res, 401, '未登录或凭证无效')
    return null
  }
  if (u.status !== 'NORMAL') {
    fail(res, 403, '账号受限，不能进行社区操作')
    return null
  }
  return u
}
...
async function handle(req: IncomingMessage, res: ServerResponse, pathWithQuery: string): Promise<void> {
  ...
  if (seg[0] === 'auth' && seg[1] === 'register' && method === 'POST') {
    ...
    users.push(u)
    return ok(res, authResult(u), 201)
  }
  ...
  if (seg[0] === 'posts' && !seg[1]) {
    ...
    if (method === 'GET') {
      ...
      let list = posts.filter(isPubliclyVisible)
      if (keyword) list = list.filter((post) => post.content.toLowerCase().includes(keyword))
      list = [...list].sort((a, b) => b.createdAt.localeCompare(a.createdAt))
      const total = list.length
      const start = (page - 1) * pageSize
      return ok(res, {
        items: list.slice(start, start + pageSize).map((post) => publicPost(post, u.id)),
        page,
        pageSize,
        total,
      })
    ...
    if (method === 'POST') {
      const writer = requireCommunityWriter(res, u)
      if (!writer) return
      const body = await readBody(req)
      const checkInId = Number(body.checkInId)
      const content = String(body.content ?? '').trim()
      const visibility = body.visibility === 'PRIVATE' ? 'PRIVATE' : 'PUBLIC'
      if (!checkInId) return fail(res, 400, 'checkInId 不能为空')
      if (!content) return fail(res, 400, '动态内容不能为空')
      const checkin = checkins.find((item) => item.id === checkInId && item.userId === writer.id)
      if (!checkin) return fail(res, 404, '打卡不存在或不可见')
      if (!checkin.completed) return fail(res, 400, '只有已完成打卡可以发布动态')
      const existing = posts.find((post) => post.checkInId === checkInId)
      if (existing) return ok(res, publicPost(existing, writer.id))
      ...
      posts.push(post)
      return ok(res, publicPost(post, writer.id), 201)
    }

    return fail(res, 405, '方法不支持')
  }

  if (seg[0] === 'posts' && seg[1] === 'search' && method === 'GET') {
    const u = currentUser(req)
    if (!u) return fail(res, 401, '未登录或凭证无效')
    const page = toNum(query.get('page')) ?? 1
    const pageSize = toNum(query.get('pageSize')) ?? 10
  ...
  if (seg[0] === 'posts' && seg[1] === 'mine' && method === 'GET') {
    const u = currentUser(req)
    if (!u) return fail(res, 401, '未登录或凭证无效')
    const page = toNum(query.get('page')) ?? 1
    const pageSize = toNum(query.get('pageSize')) ?? 10
    const list = posts
      .filter((post) => post.userId === u.id && post.status !== 'DELETED')
  ...
  if (seg[0] === 'posts' && seg[1]) {
    ...
    if (id === null) return fail(res, 400, '非法动态 id')
    const post = posts.find((item) => item.id === id)
    if (!post) return fail(res, 404, '动态不存在或不可见')

    if (seg[2] === 'likes') {
      const writer = requireCommunityWriter(res, u)
      if (!writer) return
      if (!isPubliclyVisible(post)) return fail(res, 404, '动态不存在或不可见')
      if (method === 'POST' && !postLikes.some((item) => item.userId === writer.id && item.postId === post.id)) {
        postLikes.push({ userId: writer.id, postId: post.id })
      }
      if (method === 'DELETE') {
        const idx = postLikes.findIndex((item) => item.userId === writer.id && item.postId === post.id)
        if (idx >= 0) postLikes.splice(idx, 1)
      }
      if (method === 'POST' || method === 'DELETE') {
        return ok(res, {
          postId: post.id,
          liked: postLikes.some((item) => item.userId === writer.id && item.postId === post.id),
          likeCount: postLikes.filter((item) => item.postId === post.id).length,
        })
      }
      return fail(res, 405, '方法不支持')
    ...
    if (seg[2] === 'comments') {
      if (method === 'GET') {
        if (post.userId !== u.id && !isPubliclyVisible(post)) return fail(res, 404, '动态不存在或不可见')
        return ok(res, comments
          .filter((item) => item.postId === post.id && item.status === 'VISIBLE')
          .map(publicComment))
      }
      if (method === 'POST') {
        const writer = requireCommunityWriter(res, u)
        ...
        return ok(res, publicComment(comment), 201)
      }
      return fail(res, 405, '方法不支持')
    }

    if (method === 'GET') {
      if (post.userId === u.id && post.status !== 'DELETED') return ok(res, publicPost(post, u.id))
      if (!isPubliclyVisible(post)) return fail(res, 404, '动态不存在或不可见')
      return ok(res, publicPost(post, u.id))
    }

    if (method === 'PATCH') {
      const writer = requireCommunityWriter(res, u)
      if (!writer) return
      if (post.userId !== writer.id || post.status === 'DELETED') return fail(res, 404, '动态不存在或不可见')
      const body = await readBody(req)
      const content = typeof body.content === 'string' ? body.content.trim() : ''
      if ('content' in body && !content) return fail(res, 400, '动态内容不能为空')
      if (content) post.content = content
      if (body.visibility === 'PUBLIC' || body.visibility === 'PRIVATE') post.visibility = body.visibility
      post.updatedAt = new Date().toISOString()
      return ok(res, publicPost(post, writer.id))
    }
  ...
  if (seg[0] === 'comments' && seg[1] && method === 'DELETE') {
    const writer = requireCommunityWriter(res, currentUser(req))
    if (!writer) return
    const id = toNum(seg[1])
    if (id === null) return fail(res, 400, '非法评论 id')
    const comment = comments.find((item) => item.id === id && item.userId === writer.id)
    if (!comment || comment.status === 'DELETED') return fail(res, 404, '评论不存在或不可见')
    comment.status = 'DELETED'
    return ok(res, null)
  }

  if (seg[0] === 'users' && seg[1] && seg[2] === 'follow') {
    const writer = requireCommunityWriter(res, currentUser(req))
    if (!writer) return
    const targetId = toNum(seg[1])
    if (targetId === null) return fail(res, 400, '非法用户 id')
    if (targetId === writer.id) return fail(res, 400, '不能关注自己')
    const target = users.find((item) => item.id === targetId && item.status === 'NORMAL')
    if (!target) return fail(res, 404, '用户不存在或不可见')
    if (method === 'POST' && !follows.some((item) => item.followerId === writer.id && item.followingId === targetId)) {
      follows.push({ followerId: writer.id, followingId: targetId })
    }
    if (method === 'DELETE') {
      const idx = follows.findIndex((item) => item.followerId === writer.id && item.followingId === targetId)
      if (idx >= 0) follows.splice(idx, 1)
    }
    if (method === 'POST' || method === 'DELETE') {
      return ok(res, {
        userId: targetId,
        following: follows.some((item) => item.followerId === writer.id && item.followingId === targetId),
      })
    }
    return fail(res, 405, '方法不支持')
  }
