      ...
      <el-col :span="10">
        <el-card shadow="never">
          ...
          <el-form :model="form" label-width="90px">
            ...
            <el-form-item v-if="isMockMode" label="打卡图片">
            ...
            <el-alert
              ...
              :closable="false"
              title="图片上传暂未接入"
              description="当前连接的后端暂不提供上传接口，文字打卡仍可正常保存。"
              class="upload-notice"
            />
            <el-form-item>
              <el-button
...
import type { CheckinPayload } from '@/api/checkins'
...
async function onSave() {
  ...
  try {
    const payload: CheckinPayload = {
      ...
      note: form.note || null,
      // The current backend persists imageUrl even though it does not yet host uploads.
      // Sending the existing value prevents an externally supplied image from being cleared.
      imageUrl: form.imageUrl,
    }
    await upsertCheckin(planId.value, checkDate.value, payload)
